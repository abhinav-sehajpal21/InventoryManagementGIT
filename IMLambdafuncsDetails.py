...
def lambda_handler(event, context):
    ...
    # Write the function details to a CSV file
    IST = datetime.timezone(datetime.timedelta(hours=5, minutes=30))
    filename = f"/tmp/Lambda-Inventory-{datetime.datetime.now(IST).strftime('%d-%m-%Y-%H-%M-%S')}.csv"
    ...
    return {
        'statusCode': 200,
        'body': f'Lambda Inventory file "{filename}" successfully generated and uploaded to S3 bucket "{bucket_name}"'
    }
